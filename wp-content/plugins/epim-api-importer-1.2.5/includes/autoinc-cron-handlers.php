                    ...
                    if ($atName) {
                        ...
                        if ($WCAttribute) {
                            if (!property_exists($WCAttribute, 'id')) {
                                ...
                                $attributeWCslug = apply_filters('epim_sort_attributes_attributeWCslug', $attributeWCslug);
                                $product_attribute = apply_filters('epim_sort_attributes_product_attribute', $product_attribute, $attributeWCslug);
                                $current_terms = apply_filters('epim_sort_attributes_current_terms',$current_terms,$attributeWCslug);
    ...
    foreach ($product_posts as $product_post) {
        ...
        if ($variation) {
            ...
            foreach ($variation->AttributeValues as $attribute_value) {
                ...
                $product_attribute = apply_filters('epim_link_attributes_product_attribute', $product_attribute, $attributeWCslug);
                ...
                $av_compare = str_replace('\u00b0', 'º', $av_compare);
                $av_compare = str_replace('\u2122', '™', $av_compare);
                $av_compare = trim($av_compare);
                if (strlen($av_compare) > 100) {
                    $av_compare = substr($av_compare, 0, 99);
                }
                $av_compare = strtolower($av_compare);
                $av_compare = trim(preg_replace('/\s+/', ' ', $av_compare));
                $av_compare = str_replace('  ', ' ', $av_compare);
                ...
                if (is_wp_error($wc_taxonomy_name_terms)) {
                    ...
                    if (is_array($wc_taxonomy_name_terms)) {
                        ...
                        foreach ($wc_taxonomy_name_terms as $term) {
                            if (strtolower($term->name) == $av_compare) {
                                $product_term = array();
                                $product_term['id'] = $term->term_id;
                        ...
                        if(!$term_found) {
                            foreach ($wc_taxonomy_name_terms as $term) {
                                if (strtolower($term->name) == stripslashes($av_compare)) {
                                    $product_term = array();
                                    $product_term['id'] = $term->term_id;
                                    $product_term['name'] = $av_compare;
                                    $product_term['slug'] = $term->slug;
                        ...
                        if(!$term_found) {
                            foreach ($wc_taxonomy_name_terms as $term) {
                                if(str_starts_with(strtolower($term->name),$av_compare)) {
                                    $product_term = array();
                                    $product_term['id'] = $term->term_id;
                                    $product_term['name'] = $av_compare;
                                    $product_term['slug'] = $term->slug;
                                    ...
                                    if(str_starts_with($av_compare,strtolower($term->name))) {
                                        $product_term = array();
                                        $product_term['id'] = $term->term_id;
                                        $product_term['name'] = $av_compare;
                                        $product_term['slug'] = $term->slug;
                        ...
                        if(!$term_found) {
                            //$this_product = wc_get_product($product_post);
                            cron_log('Product: '. $product_post);
                            cron_log('Attribute: '.$product_attribute['taxonomy_name'].' ('.$attribute_value->AttributeHeaderName.')');
                            cron_log('no term found for: ' . $av_compare);
                        }
