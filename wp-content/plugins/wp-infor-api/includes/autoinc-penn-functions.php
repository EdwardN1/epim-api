		...
		if ( in_array( 'customer', $roles ) ) {
			...
			if ( is_array( $old_shipping_details ) ) {
				...
				if ( $shipping_id == '0' ) {
					...
					error_log( 'set_organization_shipping_details stage 1' );
				...
				foreach ( $old_shipping_details as $old_shipping_detail ) {
					if ( $old_shipping_detail['delivery-CSD-ID'] == $shipping_id ) {
						...
						foreach ( $array_keys as $array_key ) {
							if ( wpiai_convert_shipping_field_names( $array_key ) ) {
								...
								error_log( 'Field name ' . $array_key . ' does not exist' );
							}
				...
				if ( metadata_exists( 'user', $user_id, 'wpiai_delivery_addresses' ) ) {
					...
					error_log( 'set_organization_shipping_details stage 3' );
					...
					error_log( 'set_organization_shipping_details stage 4' );
