...
function createProductsRequest( $customer, $products, $whse = '' ) {
	$wpiai_guest_customer_number = get_option( 'wpiai_guest_customer_number' );
	if ( ! is_numeric( $wpiai_guest_customer_number ) ) {
		$wpiai_guest_customer_number = 9;
	}
	...
	if ( is_array( $products ) ) {
		...
		foreach ( $products as $product ) {
			...
			if ( $whse == '' ) {
				...
				$prodStr .= '{"seqno": 1,"prod": "' . $product . '","qtyord": 1,"unit": "EACH","whse": "' . $whse . '"},';
			}
		...
		if ( $whse == '' ) {
			...
			$request .= '{"seqno": 1,"prod": "' . $products . '","qtyord": 1,"unit": "EACH","whse": "' . $whse . '"},';
		}
	}
	$request .= ']}}}';
						...
						if ( array_key_exists( 'price', $rec ) ) {
							if ( array_key_exists( 'netavail', $rec ) ) {
								...
								if ( $productID ) {
									...
									$resRec['productID'] = $productID;
									$resRec['price']     = $rec['extamt'];
									$resRec['priceInc']     = number_format($rec['extamt']*1.2,2);
									if($rec['user4']=='Y') {
