...
function epimaapi_cron_activation() {
    ...
    wp_schedule_event( time(), 'minutes_10', 'epimaapi_update_branch_stock_minutes' );
}

function epimaapi_update_branch_stock_daily() {
    $epim_enable_scheduled_updates = get_option('epim_enable_scheduled_updates');
    ...
    if($epim_update_schedule='daily') {
        if($epim_enable_scheduled_updates=1){
